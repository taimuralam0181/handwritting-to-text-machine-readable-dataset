...
from auth import render_auth_gate, render_logout_control
...
DEPARTMENT_DATASET_PATH = BASE_DIR / "dataset" / "medicine_department_seed_balanced_10000.csv"
...
APP_LOGO_PATH = BASE_DIR / "assets" / "app_logo.png"
...
if 'csv_workspace_active' not in st.session_state:
    st.session_state.csv_workspace_active = False
if 'csv_workspace_filename' not in st.session_state:
    st.session_state.csv_workspace_filename = ""
if 'csv_workspace_rows_saved' not in st.session_state:
    st.session_state.csv_workspace_rows_saved = 0
if 'csv_workspace_images_processed' not in st.session_state:
    st.session_state.csv_workspace_images_processed = 0
if 'csv_workspace_hashes' not in st.session_state:
    st.session_state.csv_workspace_hashes = []
if 'csv_workspace_last_rows' not in st.session_state:
    st.session_state.csv_workspace_last_rows = []
if 'csv_workspace_last_image' not in st.session_state:
    st.session_state.csv_workspace_last_image = ""
if 'csv_workspace_message' not in st.session_state:
    st.session_state.csv_workspace_message = None
st.session_state.single_visible_columns = [
...
st.set_page_config(
    ...
    page_icon=Image.open(APP_ICON_PATH) if APP_ICON_PATH.exists() else "OCR",
    layout="wide"
)

authenticated_user = render_auth_gate()
...
# Header
logo_data_uri = get_app_logo_data_uri()
st.markdown(
    f"""
    <div class="main-header brand-header">
        {'<img class="brand-logo" src="' + logo_data_uri + '" alt="App logo">' if logo_data_uri else ''}
        <div>
            <h1>Prescription Data Extraction Dashboard</h1>
            <p>Handwritten prescription OCR, medicine review, and department classification</p>
        </div>
    </div>
    """,
...
OPHTHALMOLOGY_KEYWORDS = (
    ...
    'eye ointment', 'ophthalmic ointment', 'eye gel', 'ophthalmic gel',
    'eye solution', 'ophthalmic solution', 'timolol', 'latanoprost', 'travoprost',
...
NON_TARGET_MEDICINE_KEYWORDS = (
    'dermasol', 'derma', 'skin', 'cream', 'lotion', 'topical',
    'clobetasol', 'betamethasone', 'hydrocortisone', 'mometasone',
    'miconazole', 'clotrimazole', 'ketoconazole', 'terbinafine',
...
def is_non_target_medication_name(medication_name):
    normalized_name = normalize_medication_name(medication_name)
    if not normalized_name:
        return False
    if is_eye_like_medication_name(normalized_name) or is_cardiology_like_medication_name(normalized_name):
        return False
    return any(keyword in normalized_name for keyword in NON_TARGET_MEDICINE_KEYWORDS)
...
def classify_department_details(medication_name):
    ...
    if is_non_target_medication_name(medication_name):
        return {'department': 'Unknown', 'department_source': 'non_target_rules', 'confidence': 0.95}
...
def append_rows_to_collection_csv(rows_df, selected_csv_name="", new_csv_name=""):
    ...
    if target_name in ("", ".csv"):
        raise ValueError("Provide a valid CSV file name.")
...
def create_or_open_workspace_csv(csv_name):
    target_path = resolve_collection_csv_path("", csv_name)
    if target_path is None:
        raise ValueError("Enter a valid CSV file name.")

    CSV_COLLECTIONS_DIR.mkdir(parents=True, exist_ok=True)
    if not target_path.exists():
        pd.DataFrame(columns=['source_image', 'medicine_name', 'department']).to_csv(target_path, index=False)
    return target_path


def get_collection_csv_row_count(target_path):
    target_path = Path(target_path)
    if not target_path.exists():
        return 0
    try:
        return int(len(pd.read_csv(target_path)))
    except Exception:
        return 0


def get_active_workspace_csv_path():
    if not st.session_state.get('csv_workspace_filename'):
        return None
    return resolve_collection_csv_path("", st.session_state.csv_workspace_filename)


def reset_csv_workspace_state():
    st.session_state.csv_workspace_active = False
    st.session_state.csv_workspace_filename = ""
    st.session_state.csv_workspace_rows_saved = 0
    st.session_state.csv_workspace_images_processed = 0
...
def render_collection_csv_section(rows_df, widget_prefix):
    ...
    if target_path is None:
        st.info("Select an existing CSV or enter a new CSV file name before appending.")

    if st.button(
        "Append To Selected CSV",
        use_container_width=True,
        key=f"collection_csv_append_{widget_prefix}",
        disabled=target_path is None,
    ):
        try:
            result = append_rows_to_collection_csv(rows_df, selected_csv, new_csv_name)
    ...
    if target_path is not None and target_path.exists() and st.button(
        "Clear Selected Collection",
        use_container_width=True,
        key=f"collection_csv_clear_{widget_prefix}",
    ):
        try:
...
with st.sidebar:
    ...
    st.header("Dashboard")
    render_logout_control(authenticated_user)
    st.markdown("---")
...
st.markdown(
    ...
    '<div class="control-bar-copy">Choose one image, a batch, or a CSV workspace for one-by-one saving.</div>',
    unsafe_allow_html=True,
)
workflow_mode = st.radio(
    "Workflow",
    options=["Single Image", "Bulk Images", "CSV Workspace"],
    horizontal=True,
)
...
render_stat_cards([
    {
        'label': 'Workflow',
        'value': workflow_mode,
        'subtext': 'Choose single, batch, or workspace CSV saving',
    },
    {
...
with col1:
    ...
    elif workflow_mode == "Bulk Images":
        render_dashboard_section(
            "Batch Input",
    ...
    elif workflow_mode == "CSV Workspace":
        render_dashboard_section(
            "Workspace Input",
            "CSV Workspace",
            "Create one CSV, then process prescription images one by one. Each extraction is saved to the same CSV automatically.",
        )

        if not st.session_state.csv_workspace_active:
            render_upload_helper(
                "Start a CSV workspace",
                "Enter a CSV file name first. After starting, every processed image in this workspace will append medicine rows to that CSV.",
            )
            with st.form("csv_workspace_start_form", clear_on_submit=False):
                workspace_csv_name = st.text_input(
                    "CSV file name",
                    placeholder="example: prescription_workspace.csv",
                    key="csv_workspace_name_input",
                )
                submitted = st.form_submit_button("Start Workspace", type="primary", use_container_width=True)
            if submitted:
                try:
                    target_path = create_or_open_workspace_csv(workspace_csv_name)
                    st.session_state.csv_workspace_active = True
                    st.session_state.csv_workspace_filename = target_path.name
                    st.session_state.csv_workspace_rows_saved = get_collection_csv_row_count(target_path)
                    st.session_state.csv_workspace_images_processed = 0
                    st.session_state.csv_workspace_hashes = []
                    st.session_state.csv_workspace_last_rows = []
                    st.session_state.csv_workspace_last_image = ""
                    st.session_state.csv_workspace_message = f"Workspace ready: {target_path.name}"
                    st.rerun()
                except Exception as workspace_error:
                    st.error(f"Could not start workspace: {workspace_error}")
        else:
            active_csv_path = get_active_workspace_csv_path()
            st.success(f"Active CSV: {st.session_state.csv_workspace_filename}")
            st.caption("Upload one prescription image, extract it, then upload the next image. Rows will keep saving to the active CSV.")

            workspace_file = st.file_uploader(
                "Select next prescription image",
                type=['jpg', 'jpeg', 'png'],
                label_visibility="collapsed",
                key="csv_workspace_image_uploader",
            )

            workspace_extraction_mode = st.radio(
                "Workspace extraction mode",
                options=[
                    "Use full image",
                    "Use auto crop",
                    "Use both and keep better result",
                ...
                index=0,
                key="csv_workspace_extraction_mode",
                help="Use this extraction strategy for each image saved into the workspace CSV.",
            )

            if workspace_file:
                workspace_bytes = workspace_file.getvalue()
                workspace_hash = compute_bytes_hash(workspace_bytes)
                duplicate_workspace_image = workspace_hash in st.session_state.csv_workspace_hashes
                workspace_image = Image.open(io.BytesIO(workspace_bytes)).convert('RGB')
                render_file_meta(workspace_file.name, workspace_file.size / 1024, "Workspace image")
                st.image(workspace_image, use_column_width=True, caption="Next Workspace Image")
                if duplicate_workspace_image:
                    st.warning("This image was already saved in this workspace. Enable duplicate saving only if you need another copy.")
                allow_duplicate_workspace = st.checkbox(
                    "Allow saving this duplicate image",
                    value=False,
                    key="csv_workspace_allow_duplicate",
                    disabled=not duplicate_workspace_image,
                )

                can_save_workspace_image = not duplicate_workspace_image or allow_duplicate_workspace
                if st.button(
                    "Extract and Save to Workspace CSV",
                    type="primary",
                    use_container_width=True,
                    key="csv_workspace_extract_button",
                    disabled=not can_save_workspace_image,
                ):
                    progress_bar = st.progress(0, text="Starting workspace extraction... 0%")
                    try:
                        if not GEMINI_API_KEY:
                            st.error("System configuration error. Please contact support.")
                            st.stop()
                        if active_csv_path is None:
                            st.error("No active workspace CSV found. Start a workspace again.")
                            st.stop()

                        progress_bar.progress(15, text="Preparing image... 15%")
                        selected_result, _, used_cache = run_cached_extraction(
                            workspace_bytes,
                            workspace_file.type or "image/jpeg",
                            workspace_extraction_mode,
                            workspace_hash,
                            image=workspace_image,
                        )
                        st.session_state.last_analysis_from_cache = used_cache
                        progress_bar.progress(75, text="Saving rows to CSV... 75%")

                        clean_rows = build_clean_medication_rows(selected_result.get('medications', []))
                        workspace_rows = []
                        for row in clean_rows:
                            workspace_rows.append({
                                'source_image': workspace_file.name,
                                'medicine_name': row.get('medicine_name', ''),
                                'department': row.get('department', 'Unknown'),
                            })

                        if workspace_rows:
                            result = append_rows_to_collection_csv(
                                pd.DataFrame(workspace_rows),
                                selected_csv_name=st.session_state.csv_workspace_filename,
                            )
                            st.session_state.csv_workspace_rows_saved = int(result['total_rows'])
                            st.session_state.csv_workspace_images_processed += 1
                            st.session_state.csv_workspace_hashes.append(workspace_hash)
                            st.session_state.csv_workspace_last_rows = workspace_rows
                            st.session_state.csv_workspace_last_image = workspace_file.name
                            st.session_state.csv_workspace_message = (
                                f"Saved {result['rows_written']} row(s) from {workspace_file.name} to {result['filename']}."
                            )
                            add_processing_history_entry(
                                "CSV Workspace",
                                workspace_file.name,
                                workspace_hash,
                                pd.DataFrame(workspace_rows).rename(columns={'source_image': 'filename'}),
                            )
                            st.session_state.total_processed += 1
                            progress_bar.progress(100, text="Saved to CSV 100%")
                            st.rerun()
                        else:
                            progress_bar.progress(100, text="Completed 100%")
                            st.warning("No medicine rows were detected, so nothing was saved to the CSV.")
                    except Exception as workspace_error:
                        progress_bar.empty()
                        st.error(f"Workspace processing error: {workspace_error}")
...
with col2:
    ...
    if workflow_mode == "Single Image":
        ...
        render_dashboard_section(
            "Workspace Output",
            "Active CSV Status",
            "Monitor the active CSV, preview the latest saved rows, and download the growing workspace file.",
        )
    ...
    if workflow_mode == "Single Image" and st.session_state.processing_complete and st.session_state.extracted_text:
        ...
        if not st.session_state.csv_workspace_active:
            render_empty_state(
                "No CSV workspace started",
                "Enter a CSV file name on the left and click Start Workspace.",
            )
        else:
            if st.session_state.csv_workspace_message:
                st.success(st.session_state.csv_workspace_message)
                st.session_state.csv_workspace_message = None

            active_csv_path = get_active_workspace_csv_path()
            if active_csv_path is not None:
                st.markdown("#### Workspace Status")
                status_rows = [
                    {
                        'Metric': 'Active CSV',
                        'Value': st.session_state.csv_workspace_filename,
                    },
                    {
                        'Metric': 'Images processed',
                        'Value': st.session_state.csv_workspace_images_processed,
                    },
                    {
                        'Metric': 'Total rows saved',
                        'Value': get_collection_csv_row_count(active_csv_path),
                    },
                ]
                safe_dataframe(pd.DataFrame(status_rows), use_container_width=True, hide_index=True)

                if st.session_state.csv_workspace_last_rows:
                    st.markdown("#### Latest Saved Rows")
                    safe_dataframe(
                        pd.DataFrame(st.session_state.csv_workspace_last_rows),
                        use_container_width=True,
                        hide_index=True,
                    )
                else:
                    st.info("No image has been saved in this workspace yet.")

                st.markdown("---")
                if active_csv_path.exists():
                    st.download_button(
                        label="Download Workspace CSV",
                        data=read_collection_csv_bytes(active_csv_path),
                        file_name=active_csv_path.name,
                        mime="text/csv",
                        use_container_width=True,
                        key="csv_workspace_download_button",
                    )
                else:
                    st.warning("The active CSV file is missing. Start a new workspace.")

                if st.button("End Workspace", use_container_width=True, key="csv_workspace_end_button"):
                    reset_csv_workspace_state()
                    st.rerun()
